...
const updateByIdMember = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const data = req.body;
  const result = await MemberService.updateByIdMember(id, data);
...
const deleteByIdMember = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await MemberService.deleteByIdMember(id);
